...
class BertNLIModel(nn.Module):
    ...
    def __init__(self,model_path=None,gpu=True,bert_type='bert-large',label_num=3,batch_size=8,reinit_num=0,freeze_layers=False):
        ...
        if 'bert-base' in bert_type:
            ...
            #self.bert = BertForPretrainng.from_pretrained('bert-base-uncased')
            ...
            self.tokenizer = BertTokenizer.from_pretrained('bert-large-uncased',config=config)
    ...
    def step_bert_encode(self, module, hidden_states, attention_mask=None, head_mask=None):
        ...
        for i, layer_module in enumerate(module.layer):
            ...
            #if module.output_hidden_states==True:
    ...
    def step_checkpoint_bert(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None):
        #print(self.bert)
        #print(self.bert._modules.items())
    ...
    def ff(self,sent_pair_list,checkpoint):
        ...
        # del types_tensor
        # del masks_tensor
        # torch.cpu.empty_cache() # releases all unoccupied cached memory
    ...
    @staticmethod
    def load(input_path,gpu=True,bert_type='bert-large',label_num=3,batch_size=16):
        #print('asdfgh')
        if gpu:
